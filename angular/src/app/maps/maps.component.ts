...
export class MapsComponent implements OnChanges {
  ...
  displayDest: any;   // per gestire il marker del torneo vicino che si vuole visualizzare
  display: any;       // per gestire il marker della propria posizione attuale
  defaultCenter: google.maps.LatLngLiteral = {lat: 41.8719, lng: 12.5674};
  center: google.maps.LatLngLiteral = {lat: 41.8719, lng: 12.5674};
  ...
  this.getGeocoding(place).subscribe((response: any) => {
    ...
    if (response.results.length > 0 && response.results[0].geometry != undefined) {
      ...
      */
      ...
      this.raiseError("Nessun risultato.")
    }
  });
}
